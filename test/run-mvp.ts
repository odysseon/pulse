...
async function runMvpTest() {
  ...
  try {
    ...
    const { id: businessId, slug: businessSlug } = (await verifyRes.json()) as {
      id: string;
      slug: string;
    };
    ...
    const categories = (await catRes.json()) as { children: { id: string }[] }[];
    const categoryId = categories[0]?.children[0]?.id;
    ...
    const { id: listingId } = (await listRes.json()) as { id: string };
    ...
    const { items } = (await getListingsRes.json()) as { items: { id: string }[] };
    if (!items.find((i) => i.id === listingId))
